-- Allow any authenticated student to join a class by join code (security definer; bypasses org-only RLS).
...
begin
  ...
  join public.enrollments e on e.student_id = s.id and e.class_id = class_row.id
  where s.organization_id = class_row.organization_id
